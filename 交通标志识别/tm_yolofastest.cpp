 ...
 *
 * 主要根据get_input_data_darknet函数，修改了一个能够直接使用cv::mat的函数get_input_data_yolo, 使其能够循环处理opencv捕获的数据,并提升了大概2s的处理速度
 */
...
string outputVideoPath = "/home/openailab/data/test.avi";
...
static const char* class_names[] ={
	"none","red_light","turn_left", "turn_right", "stop" 
};
...
void get_input_data_yolo(Mat img, float* input_data, int img_h, int img_w)
{
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	outputVideo.open(outputVideoPath,VideoWriter::fourcc('M','J','P','G'), 10, Size(frame_width,frame_height), true);
...
while(1)
{
	...
	get_input_data_yolo(frame, input_data.data(), net_h, net_w);
    ...
    //result_file.open("/tmp/output", ios::trunc);
    ...
    run_yolo(graph, boxes, net_w, net_h);

    for (int i = 0; i < (int)boxes.size(); ++i)
    {
        BBoxRect b = boxes[i];
        cout << class_names[b.label] << endl;
	result_file << class_names[b.label] << "\n";
    }

//	outputVideo.write(frame);
    imshow("yolofast", frame);
    result_file.close();
    if (char(waitKey(1)) == 'q')
	    break;
